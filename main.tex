...
it doesn't need induction-induction so we can obtain $\con_{\flatten{\Gamma}} : \flatten{\Gamma}^\CC$:
\begin{lemma}
...
Like with \grm{\flatten{}}, we define a transformation on contexts, types, and
...
\anntwo{y}{\underline{a}}		&:\equiv \underline{\anntwo{y}{a}} \\
\anntwo{y}{(x : a) \to B}		&:\equiv
	\begin{cases}
	\grm{(\blm{\bar{x} : \flatten{a}}) \to \annotate{B}}						& \text{for \grm{B :: \Sc}} \\
	\begin{minipage}[c]{.44\textwidth} \grm{(\blm{\bar{x} : \flatten{a}})(\annotate{\blm{\bar{x}}, a})} 
		\\ \hphantom{\qquad} \grm{\hfill{} \to \anntwo{y(\bar{x})}{B}} \end{minipage}		& \text{for \grm{B :: \Pc}} \\
	\end{cases} \\
\anntwo{y}{(\blm{x : A}) \to B}	&:\equiv
	\begin{cases}
	\grm{\blm{A} \to \annotate{B}} 			& \text{for \grm{B :: \Sc}} \\
...
\anntwo{y}{t(u)}			&:\equiv \anntwo{y}{t} (\flatten{u}) \\
\anntwo{y}{t(\blm{u})}			&:\equiv \anntwo{y}{t} (\blm{u}) \\
\anntwo{y}{(\blm{x : A}) \to b}		&:\equiv \grm{(\blm{x : A}) \to \anntwo{y(x)}{b}} \\
\anntwo{y}{t \app \blm{u}}		&:\equiv \anntwo{y}{t} \app \blm{u}
\end{align*}
\end{defn}
...
\begin{align*}
\grm{W_{\mathop{Ty}} :} 
&\phantom{\equiv~} \grm{\blm{\overline{Ty}} \to \annotate{Ty, \mathop{Con} \to \UU} } \\
&\equiv \grm{\blm{\overline{Ty}} \to \flatten{\mathop{Con}} \to \annotate{Ty, \UU}} \\
&\equiv \grm{\blm{\overline{Ty}} \to \blm{\overline{Con}} \to \UU} \text{.}
\end{align*}
The point constructor \grm{\mathop{pi} : (\Gamma : \mathop{Con})(A : \mathop{Ty}(\Gamma))(B : \mathop{Ty}(\mathop{ext}(\Gamma, A))) \to \mathop{Ty}(\Gamma)} becomes the following:
\begin{align*}
\grm{w_{\mathop{pi}} :}
& \phantom{\equiv~} \grm{(\Gamma : \mathop{Con})(A : \mathop{Ty}(\Gamma))(B : \mathop{Ty}(\mathop{ext}(\Gamma, A))) \to \mathop{Ty}(\Gamma)} \\
& \equiv \grm{(\blm{\overline{\Gamma} : \overline{Con}})(\annotate{\Gamma, \mathop{Con}}) \to foo}
\end{align*}
\end{example}
...
\begin{proof}
TODO: Somehow by recursion on \grm{\Gamma}...
\end{proof}
