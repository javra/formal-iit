...
\end{align*}
together with a type family $\mathop{Ty} : \mathop{Con} \to \UU$ with constructors
\begin{align*}
...
\flatten(\grm{\UU})			&:\equiv \grm{\UU} \\
\flatten(\grm{\underline{a}})		&:\equiv \grm{\underline{\blm{\flatten({\grm{a}})}}} \\
\flatten(\grm{(x : a) \to B})		&:\equiv
	\begin{cases}
	\flatten(\grm{B}) & \text{for \grm{B :: \Sc},} \\
	\grm{(x : \blm{\flatten(\grm{a})}) \to \blm{\flatten(\grm{B})}} & \text{for \grm{B :: \Pc}}
	\end{cases} \\
\flatten(\grm{(\blm{x : A}) \to B})	&:\equiv \grm{(\blm{x : A}) \to \blm{\flatten(\grm{B})}} \\
\flatten(\grm{x})				&:\equiv \grm{\bar{x}} \qquad \text{for variables \grm{x}} \\
\flatten(\grm{t(u)})			&:\equiv
	\begin{cases}
	\flatten(\grm{t}) & \text{for \grm{t : (x : a) \to B :: \Sc},} \\
	(\flatten(\grm{t}))\grm{(\blm{\flatten(\grm{u})})} & \text{for \grm{t : (x : a) \to B :: \Pc}}
	\end{cases} \\
\flatten(\grm{t(\blm{u})})		&:\equiv (\flatten(\grm{t}))\grm{(\blm{u})} \\
\flatten(\grm{(\blm{x : A}) \to b})	&:\equiv \grm{(\blm{x : A}) \to \blm{\flatten(\grm{b})}} \\
\flatten(\grm{t \app \blm{u}})		&:\equiv (\flatten(\grm{t}))\grm{(\blm{u})}
\end{align*}
\end{defn}
...
As an example, consider the TODO
...
\begin{equation*}
\begin{gathered}
\inferrule{\grm{\vdash \Gamma}}{\grm{\vdash \blm{\flatten(\grm{\Gamma})}}}
\qquad
\inferrule{\grm{\Gamma \vdash A}}{\grm{\blm{\flatten(\grm{\Gamma})} \vdash \blm{\flatten(\grm{A})}}}
\\
\inferrule{\grm{\Gamma \vdash t : A}}{\grm{\blm{\flatten(\grm{\Gamma})} \vdash \blm{\flatten(\grm{t})} : \blm{\flatten(\grm{A})}}}
\end{gathered}
\end{equation*}
\end{lemma}

\begin{proof}
Let us only consider the cases where $\flatten$ acts neither as the identity nor
by trivial structural descent:
